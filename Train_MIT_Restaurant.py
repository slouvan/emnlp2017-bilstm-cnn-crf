...
model.storeResults('results/MIT_Restaurant_SingleTask_'+str(datasets['MIT_Restaurant']['proportion'])+'_slot_results.csv') #Path to store performance scores for dev / test
model.predictionSavePath = "results/[ModelName]_SingleTask_"+str(datasets['MIT_Restaurant']['proportion'])+"_[Epoch]_[Data].conll" #Path to store predictions
model.modelSavePath = "models/[ModelName]_SingleTask_"+str(datasets['MIT_Restaurant']['proportion'])+"_[DevScore]_[TestScore]_[Epoch].h5" #Path to store models
model.fit(epochs=50)
